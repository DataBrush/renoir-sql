use crate::ir::ast_builder::ir_ast_structure::ComplexField;
use crate::ir::{ExistsCondition, InCondition};
use crate::query::subquery_process::*;
use crate::{
    ir::{
...
fn process_scan(input: &Arc<IrPlan>, query_object: &mut QueryObject) -> Arc<IrPlan> {
    ...
    query_object.tables_info.insert(
        ...
        stream_info
            .final_struct
            .get(stream_info.final_struct.keys().last().unwrap())
            .unwrap()
            .clone(),
    );
