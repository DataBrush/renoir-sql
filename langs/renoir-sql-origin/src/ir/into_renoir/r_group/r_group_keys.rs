use crate::ir::ir_ast_structure::{AggregateType, GroupClause};
use crate::ir::r_group::conditions::{
    r_group_conditions::parse_group_conditions, r_group_filter::create_filter_operation,
    r_group_fold::create_fold_operation,
};
use crate::ir::r_utils::check_alias;
use crate::ir::{AggregateFunction, ProjectionColumn};
use crate::ir::{ColumnRef, QueryObject};
...
pub(crate) fn process_group_by(
    ...
    if let Some(condition) = group_condition {
            ...
            .map(|col| {
                let stream_name = if col.table.is_some() {
                    ...
                    if all_streams.len() == 1 {
                        ...
                        panic!(
                            "Column {} does not have a table reference in a join query",
                            col.column
                        );
                    }
                };
